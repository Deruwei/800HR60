...
public class VerificationCodeView extends RelativeLayout {
    ...
    private void init(final Context context, AttributeSet attrs, int defStyleAttr) {
        View view=LayoutInflater.from(context).inflate(R.layout.layout_identifying_code, this);
        containerEt = view.findViewById(R.id.container_et);
    ...
    private void initPopupWindow(Activity context) {
        View viewIndustry =context.getLayoutInflater().inflate(R.layout.layout_copy_paste, null);
        popupWindow = new PopupWindow(viewIndustry, LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.MATCH_PARENT);
        TextView tv_paste=viewIndustry.findViewById(R.id.tv_paste);
        int[] location = new int[2];
        containerEt.getLocationOnScreen(location);
        int x = location[0];
        int y = location[1];
        WindowManager wm = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
        DisplayMetrics dm = new DisplayMetrics();
        wm.getDefaultDisplay().getMetrics(dm);
        tv_paste.setOnClickListener(new OnClickListener() {
            @Override
