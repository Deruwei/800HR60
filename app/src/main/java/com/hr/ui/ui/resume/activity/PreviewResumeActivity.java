...
public class PreviewResumeActivity extends BaseNoConnectNetworkAcitivty {
    ...
    private void initPlantExp() {
        if(plantListBeanList!=null&&plantListBeanList.size()!=0){
            for(int i=0;i<plantListBeanList.size();i++){
                ...
                tvTime.setText(plantListBeanList.get(i).getFromyear()+"年"+plantListBeanList.get(i).getFrommonth()+"月"+"—"+plantListBeanList.get(i).getToyear()+"年"+plantListBeanList.get(i).getTomonth()+"月");
    ...
    private void initProfessionSkill() {
        if(skillListBeanList!=null&&skillListBeanList.size()!=0){
            for(int i=0;i<skillListBeanList.size();i++){
                View view= LayoutInflater.from(this).inflate(R.layout.item_professionskill, null);
                // initview
    ...
    private void initBaseInfo() {
        ...
        Glide.with(this).load(Constants.IMAGE_BASEPATH +baseInfoBean.get(0).getPic_filekey()).centerCrop().into(ivPreviewResumePhoto);
    }
