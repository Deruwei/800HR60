...
public class ToastUitl {
    ...
    private static Toast initToast(CharSequence message, int duration) {
        if (toast == null) {
            toast = Toast.makeText(HRApplication.getAppContext(), message, duration);
        } else {
...
//		Toast.makeText(context, strResId, Toast.LENGTH_SHORT).show();
    ...
    public static void showLong(int strResId) {
        initToast(HRApplication.getAppContext().getResources().getText(strResId), Toast.LENGTH_LONG).show();
    }
    ...
    public static Toast showToastWithImg(final String tvStr, final int imageResource) {
        if (toast2 == null) {
            toast2 = new Toast(HRApplication.getAppContext());
        }
        View view = LayoutInflater.from(HRApplication.getAppContext()).inflate(R.layout.toast_custom, null);
